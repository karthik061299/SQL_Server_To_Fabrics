...
AS
BEGIN
    -- Temporary table naming - using Fabric-compatible temp tables
    ...
    -- In Fabric SQL, we'll use regular temporary tables with session-specific naming
    SELECT @TabName = 'CTM_' + CAST(SESSION_ID() AS varchar(10));
    SELECT @TabNameFact = 'CTMFact_' + CAST(SESSION_ID() AS varchar(10));
    SELECT @TabFinal = 'CTMF_' + CAST(SESSION_ID() AS varchar(10));
    SELECT @TabNamePrs = 'CTPrs_' + CAST(SESSION_ID() AS varchar(10));
    SELECT @ProdSource = 'PRDCLmTrans_' + CAST(SESSION_ID() AS varchar(10));
    
    -- SET NOCOUNT ON added to prevent extra result sets from
    -- interfering with SELECT statements.
    SET NOCOUNT ON;
    
    -- Fabric SQL doesn't support XACT_ABORT, removing this line
    ...
    -- Modify system table query for Fabric compatibility
    SET @CATCount =
    (
    ...
    -- In Fabric SQL, we don't need to disable indexes before bulk operations
    -- Removing all the index disable code
